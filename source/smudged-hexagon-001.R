...
tic()
dat1 <- smudged_hexagon(seed = 88) |>
  mutate(fill = "#D00070", id = paste0("hex1_", id), x = x + 1)

dat2 <- smudged_hexagon(seed = 91) |>
  mutate(fill = "#8C4799", id = paste0("hex2_", id), x = x + 2)

dat3 <- smudged_hexagon(seed = 10) |>
  mutate(fill = "#0032A0", id = paste0("hex3_", id), x = x + 3)

dat <- bind_rows(dat1, dat2, dat3)

pic <- ggplot(dat, aes(x, y, group = id, fill = fill)) +
  geom_polygon(colour = NA, alpha = .02, show.legend = FALSE) + 
  scale_fill_identity() +
  coord_equal(xlim = c(-1, 4), ylim = c(-2.5, 2.5)) + 
  theme_void()
